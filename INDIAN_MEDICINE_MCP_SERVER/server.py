...
DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "medicines.json")
...
def name_match_score(query: str, medicine_name: str) -> float:
    """Score exact, partial, and lightly misspelled medicine names."""
    normalized_query = re.sub(r"[^a-z0-9]+", " ", query.lower()).strip()
    normalized_name = re.sub(r"[^a-z0-9]+", " ", medicine_name.lower()).strip()
    ...
    token_scores = [
        max(similarity_score(query_token, name_token) for name_token in name_tokens)
        for query_token in query_tokens
    ]
    token_score = sum(token_scores) / len(token_scores)
    sequence_score = similarity_score(normalized_query, normalized_name)
...
def fuzzy_search_by_name(partial_name: str, similarity_threshold: float = 0.6, max_results: int = 10) -> str:
    ...
    scored_results = []
    for entry in medicines:
        med_name = entry.get("Name", "")
        if not med_name:
            continue
        score = name_match_score(partial_name, med_name)
